 ...
 *
 */
interface IBerryProxy is IBerryNetworkProxy, ISimpleBerryProxy {
    // empty block
}


/**
 * @title berryFeeHandler
 *
 * @dev berryFeeHandler works tightly with contracts berryNetwork and berryDao.
 *      Some events are moved to interface, for easier usage
 * @dev Terminology:
 *          Epoch - Voting campaign time frame in berryDao.
 *              berryDao voting campaigns are in the scope of epochs.
 *          BRR - Burn / Reward / Rebate. berryNetwork fee is used for 3 purposes:
 *              Burning FIB
 *              Reward an address that staked fib in berryStaking contract. AKA - stakers
 *              Rebate reserves for supporting trades.
 * @dev Code flow:
 *      1. Accumulating && claiming Fees. Per trade on berryNetwork, it calls handleFees() function which
 *          internally accounts for network & platform fees from the trade. Fee distribution:
 *              rewards: accumulated per epoch. can be claimed by the berryDao after epoch is concluded.
 ...
 *      2. Network Fee distribution: Per epoch berryFeeHandler contract reads BRR distribution percentage 
 *          from berryDao. When the data expires, berryFeeHandler reads updated values.
 */
contract BerryFeeHandler is IBerryFeeHandler, Utils5, DaoOperator, ReentrancyGuard {
    ...
    IBerryDao public berryDao;
    IBerryProxy public berryProxy;
    address public berryNetwork;
    IERC20 public immutable fib;
    ...
    /// @dev amount of eth to burn for each burn fib call
    ...
    /// @dev use to get rate of FIB/ETH to check if rate to burn fib is normal
    ...
    event BerryDaoAddressSet(IBerryDao berryDao);
    ...
    event BerryNetworkUpdated(address berryNetwork);
    event BerryProxyUpdated(IBerryProxy berryProxy);

    constructor(
        address _daoSetter,
        IBerryProxy _berryProxy,
        ...
        require(_berryProxy != IBerryProxy(0), "berryNetworkProxy 0");
        require(_berryNetwork != address(0), "berryNetwork 0");
        require(_fib != IERC20(0), "fib 0");
        require(_burnBlockInterval != 0, "_burnBlockInterval 0");

        daoSetter = _daoSetter;
        berryProxy = _berryProxy;
        berryNetwork = _berryNetwork;
        fib = _fib;
    ...
    modifier onlyBerryDao {
        require(msg.sender == address(berryDao), "only berryDao");
        _;
    }

    modifier onlyBerryNetwork {
        require(msg.sender == address(berryNetwork), "only berryNetwork");
        _;
    }
    ...
    /// @dev handleFees function is called per trade on berryNetwork. unless the trade is not involving any fees.
    ...
    function handleFees(
        ...
        uint256 platformFee,
        uint256 networkFee
    ) external payable override onlyBerryNetwork nonReentrant {
    ...
    function claimStakerReward(
        ...
        // if the epoch is current or in the future, berryDao will return 0 as result
        uint256 percentageInPrecision = berryDao.getPastEpochRewardPercentageInPrecision(staker, epoch);
    ...
    /// @dev set berryDao contract address once and set setter address to zero.
    /// @param _berryDao berryDao address.
    function setDaoContract(IBerryDao _berryDao) external {
        require(msg.sender == daoSetter, "only daoSetter");
        require(_berryDao != IBerryDao(0));
        berryDao = _berryDao;
        emit BerryDaoAddressSet(berryDao);

        daoSetter = address(0);
    }

    /// @dev set new berryNetwork address by daoOperator
    /// @param _berryNetwork new berryNetwork contract
    function setNetworkContract(address _berryNetwork) external onlyDaoOperator {
        require(_berryNetwork != address(0), "berryNetwork 0");
        if (_berryNetwork != berryNetwork) {
            berryNetwork = _berryNetwork;
            emit BerryNetworkUpdated(berryNetwork);
        }
    }

    /// @dev Allow to set berryNetworkProxy address by daoOperator
    /// @param _newProxy new berryNetworkProxy contract
    function setBerryProxy(IBerryProxy _newProxy) external onlyDaoOperator {
        require(_newProxy != IBerryProxy(0), "berryNetworkProxy 0");
        if (_newProxy != berryProxy) {
            berryProxy = _newProxy;
            emit BerryProxyUpdated(_newProxy);
        }
    }

    /// @dev set fib sanity rate contract and amount wei to burn
    ...
    /// @dev Burn fib. The burn amount is limited. Forces block delay between burn calls.
    /// @dev only none ontract can call this function
    /// @return fibBurnAmount amount of fib burned
    function burnFib() external onlyNonContract returns (uint256 fibBurnAmount) {
        ...
        // Get rate
        uint256 berryEthFibRate = berryProxy.getExpectedRateAfterFee(
            ETH_TOKEN_ADDRESS,
            fib,
            srcAmount,
            0,
        ...
        validateEthToFibRateToBurn(berryEthFibRate);

        // Buy some fib and burn
        fibBurnAmount = berryProxy.swapEtherToToken{value: srcAmount}(
            fib,
            berryEthFibRate
        );

        require(IBurnableToken(address(fib)).burn(fibBurnAmount), "fib burn failed");

        emit FibBurned(fibBurnAmount, ETH_TOKEN_ADDRESS, srcAmount);
        return fibBurnAmount;
    }
    ...
    ///         calls berryDao contract to check if there were any votes for this epoch.
    /// @param epoch epoch number to check.
    function makeEpochRewardBurnable(uint256 epoch) external {
        require(berryDao != IBerryDao(0), "berryDao not set");

        require(berryDao.shouldBurnRewardForEpoch(epoch), "should not burn reward");
    ...
    /// @dev return latest fib/eth rate from sanity rate contract
    function getLatestSanityRate() external view returns (uint256 fibToEthSanityRate) {
        if (sanityRateContract.length > 0 && sanityRateContract[0] != ISanityRate(0)) {
            fibToEthSanityRate = sanityRateContract[0].latestAnswer();
        } else {
            fibToEthSanityRate = 0; 
        }
    }
        ...
        )
    {
        ...
        if (now > expiryTimestamp && berryDao != IBerryDao(0)) {
            uint256 burnBps;

            (burnBps, rewardBps, rebateBps, epoch, expiryTimestamp) = berryDao
    ...
    function validateEthToFibRateToBurn(uint256 rateEthToFib) internal view {
        require(rateEthToFib <= MAX_RATE, "ethToFib rate out of bounds");
        require(rateEthToFib > 0, "ethToFib rate is 0");
        ...
        // get latest fib/eth rate from sanity contract
        uint256 fibToEthRate = sanityRateContract[0].latestAnswer();
        require(fibToEthRate > 0, "sanity rate is 0");
        require(fibToEthRate <= MAX_RATE, "sanity rate out of bounds");

        uint256 sanityEthToFibRate = PRECISION.mul(PRECISION).div(fibToEthRate);

        // rate shouldn't be SANITY_RATE_DIFF_BPS lower than sanity rate
        require(
            rateEthToFib.mul(BPS) >= sanityEthToFibRate.mul(BPS.sub(SANITY_RATE_DIFF_BPS)),
            "berryNetwork eth to fib rate too low"
        );
    }
}