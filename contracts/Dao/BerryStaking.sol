 ...
 *           Staking contract will be deployed by BerryDao's contract
 */
contract BerryStaking is IBerryStaking, EpochUtils, ReentrancyGuard {
    struct StakerData {
        uint256 stake;
    ...
    constructor(
        ...
        require(_fibToken != IERC20(0), "ctor: fibToken 0");
        require(_berryDao != IBerryDao(0), "ctor: berryDao 0");
    ...
    /**
     * @dev call to stake more FIB for msg.sender
    ...
    function deposit(uint256 amount) external override {
        ...
        // collect FIB token from staker
        require(
            fibToken.transferFrom(staker, address(this), amount),
    ...
    /**
     * @dev call to withdraw FIB from staking, it could affect reward when calling BerryDao handleWithdrawal
     * @param amount amount of FIB to withdraw
     */
    function withdraw(uint256 amount) external override nonReentrant {
        ...
        // transfer FIB back to staker
        require(fibToken.transfer(staker, amount), "withdraw: can not transfer fib");
        emit Withdraw(curEpoch, staker, amount);
    }

    /**
     * @dev initialize data if needed, then return staker's data for current epoch
     * @dev for safe, only allow calling this func from BerryDao address
    ...
    {
        require(
            msg.sender == address(berryDao),
            "initAndReturnData: only berryDao"
        );
     ...
     * @dev  in BerryDao contract, if staker wants to claim reward for past epoch,
    ...
    */
    function handleWithdrawal(
        ...
        if (reduceAmount > 0) {
            ...
            // call BerryDao to reduce reward, if staker has delegated, then pass his representative
            if (address(berryDao) != address(0)) {
                // don't revert if BerryDao revert so data will be updated correctly
                (bool success, ) = address(berryDao).call(
                    abi.encodeWithSignature(
