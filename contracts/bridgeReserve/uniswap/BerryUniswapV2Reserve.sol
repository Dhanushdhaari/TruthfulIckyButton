...
contract BerryUniswapV2Reserve is IBerryReserve, Withdrawable3, Utils5 {
    ...
    event BerryNetworkSet(address berryNetwork);
    ...
    function trade(
        ...
        require(msg.sender == berryNetwork, "only berryNetwork");
    ...
    function setBerryNetwork(address _berryNetwork) external onlyAdmin {
        require(_berryNetwork != address(0));
        if (berryNetwork != _berryNetwork) {
            berryNetwork = _berryNetwork;
            emit BerryNetworkSet(berryNetwork);
        }
    }
    ...
    /**
     *   @dev called by berrynetwork to get settlement rate
     */
