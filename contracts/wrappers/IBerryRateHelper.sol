...
import "./IBerryRateHelper.sol";
import "../IBerryDao.sol";
import "../IBerryStorage.sol";
import "../IBerryReserve.sol";
import "../utils/Utils5.sol";
import "../utils/WithdrawableNoModifiers.sol";


contract BerryRateHelper is IBerryRateHelper, WithdrawableNoModifiers, Utils5 {
    ...
    event BerryDaoContractSet(IBerryDao berryDao);
    event BerryStorageSet(IBerryStorage berryStorage);
    event AddBerryReserve(bytes32 reserveId, bool add);

    function setContracts(
        IBerryDao _berryDao,
        IBerryStorage _berryStorage
    ) public {
        onlyAdmin();
        require(_berryDao != IBerryDao(0), "berryDao 0");
        require(_berryStorage != IBerryStorage(0), "berryStorage 0");

        if (berryDao != _berryDao) {
            berryDao = _berryDao;
            emit BerryDaoContractSet(_berryDao);
        }

        if (berryStorage != _berryStorage) {
            berryStorage = _berryStorage;
            emit BerryStorageSet(_berryStorage);
        }
    }
    ...
    function addReserve(bytes32 reserveId) public {
        ...
        emit AddBerryReserve(reserveId, true);
    }
    ...
    function removeReserve(bytes32 reserveId) public {
        ...
        for (uint256 i = 0; i < reserveIds.length; i++) {
            if (reserveIds[i] == reserveId) {
                ...
                emit AddBerryReserve(reserveId, false);
                break;
            }
        }
    }
    ...
    {
        (uint256 networkFeeBps, ) = berryDao.getLatestNetworkFeeData();
    ...
    {
        (uint256 feeBps, ) = berryDao.getLatestNetworkFeeData();
    ...
    function getBuyInfo(
        ...
        buyReserves = berryStorage.getReserveIdsPerTokenDest(token);
    ...
    function getBuyRate(
        ...
        bool[] memory isFeeAccountedFlags = berryStorage.getFeeAccountedData(buyReserves);
        address[] memory buyReserveAddresses = berryStorage.getReserveAddressesFromIds(
            buyReserves
        );
        ...
        for (uint256 i = 0; i < buyReserves.length; i++) {
            if (networkFeeBps == 0 || !isFeeAccountedFlags[i]) {
                buyRates[i] = IBerryReserve(buyReserveAddresses[i]).getConversionRate(
                    ETH_TOKEN_ADDRESS,
                    token,
                    buyAmountWei,
                    block.number
    ...
    function getSellInfo(
        ...
        sellReserves = berryStorage.getReserveIdsPerTokenSrc(token);
    ...
    function getSellRate(
        ...
        bool[] memory isFeeAccountedFlags = berryStorage.getFeeAccountedData(sellReserves);
        address[] memory sellReserveAddresses = berryStorage.getReserveAddressesFromIds(
            sellReserves
        );
        sellRates = new uint256[](sellReserves.length);
        for (uint256 i = 0; i < sellReserves.length; i++) {
            sellRates[i] = IBerryReserve(sellReserveAddresses[i]).getConversionRate(
                token,
                ETH_TOKEN_ADDRESS,
                sellAmountTwei,
                block.number
